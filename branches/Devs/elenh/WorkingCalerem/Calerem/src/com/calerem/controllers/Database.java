...
public class Database extends SQLiteOpenHelper {
	...
	public Event get_event(int event_id) {
		...
		event = new Event(dbCursor.getString(dbCursor.getColumnIndex(type)), dbCursor.getString(dbCursor.getColumnIndex(name)), dbCursor.getInt(dbCursor.getColumnIndex(date)), contact,
				dbCursor.getInt(dbCursor.getColumnIndex(id)), dbCursor.getString(dbCursor.getColumnIndex(description)));
		return event;
	}
	...
	public Event[] return_events(Integer start_time, Integer end_time) {
		...
		for (int i = 0; i < dbCursor.getCount(); i++) {
			...
			events[i] = new Event(dbCursor.getString(dbCursor.getColumnIndex(type)), dbCursor.getString(dbCursor.getColumnIndex(name)), dbCursor.getInt(dbCursor.getColumnIndex(date)), contact,
					dbCursor.getInt(dbCursor.getColumnIndex(id)), dbCursor.getString(dbCursor.getColumnIndex(description)));
			dbCursor.moveToNext();
		}
		dbCursor.close();
	...
	public ConfigurationCalerem read_configuration() {
		...
		dbCursor.moveToFirst();
		ConfigurationCalerem configuration = new ConfigurationCalerem(myContext, dbCursor.getString(dbCursor.getColumnIndex(date_format)), dbCursor.getString(dbCursor.getColumnIndex(sound_path)),
				dbCursor.getString(dbCursor.getColumnIndex(language)), dbCursor.getString(dbCursor.getColumnIndex(skin_path)), dbCursor.getString(dbCursor.getColumnIndex(eortologio_url)));
		dbCursor.close();
	...
	public SyncLog[] read_sync_log(int limit) {
		...
		dbCursor.moveToFirst();
		for (int i = 0; i < dbCursor.getCount(); i++) {
			log[i] = new SyncLog(dbCursor.getInt(dbCursor.getColumnIndex(date)), dbCursor.getString(dbCursor.getColumnIndex(type)), dbCursor.getInt(dbCursor.getColumnIndex(id)));
			dbCursor.moveToNext();
		}
		dbCursor.close();
		return log;
	...
	public MessageLog[] read_message_log(int limit) {
		...
		for (int i = 0; i < dbCursor.getCount(); i++) {
			...
			log[i] = new MessageLog(dbCursor.getInt(dbCursor.getColumnIndex(date)), dbCursor.getInt(dbCursor.getColumnIndex(id)), dbCursor.getString(dbCursor.getColumnIndex(type)),
					dbCursor.getString(dbCursor.getColumnIndex(message)), contact);
			dbCursor.moveToNext();
		}
		dbCursor.close();
		return log;
	...
	public Contact get_contact(int contact_id) {
		...
		dbCursor.moveToFirst();
		Contact contact = new Contact(dbCursor.getString(dbCursor.getColumnIndex(name)), dbCursor.getString(dbCursor.getColumnIndex(lastname)), dbCursor.getString(dbCursor.getColumnIndex(phone)),
				dbCursor.getString(dbCursor.getColumnIndex(email)), dbCursor.getInt(dbCursor.getColumnIndex(id)));
		dbCursor.close();
		return contact;
	}
	...
	public Contact[] get_contacts() {
		...
		for (int i = 0; i < dbCursor.getCount(); i++) {
			contact[i] = new Contact(dbCursor.getString(dbCursor.getColumnIndex(name)), dbCursor.getString(dbCursor.getColumnIndex(lastname)), dbCursor.getString(dbCursor.getColumnIndex(phone)),
					dbCursor.getString(dbCursor.getColumnIndex(email)), dbCursor.getInt(dbCursor.getColumnIndex(id)));
		}
		dbCursor.close();
		return contact;
	...
	/**
	 * Called when the object is destroyed. Closes the database.
	 * 
