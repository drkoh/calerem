...
public class ContactsAPI {
	...
	/**
	 * Base constructor that initializes some basic value. Also queries all
	 * contacts from contact API.
	 * 
	 * @param Context
	 *            context
	 */
	...
	/**
	 * Fetches the query from contacts API Requests additional data from other
	 * functions.
	 * 
	...
	private Contact[] readDatabase() {
		Contact[] contact = new Contact[ContactsCur.getCount()];
		if (ContactsCur.getCount() > 0) {
			...
			while (ContactsCur.moveToNext()) {
				...
				contact[i] = new Contact(first_name, last_name, phone, email, (Integer) null);
				i++;
			}
		}
		ContactsCur.close();
	 ...
	 * 
	 * @param String
	 *            id Contact ID in android database.
